...
if __name__ == "__main__":
    try:
        # 文件路径
        source_file = 'D:\【交付审核】\_导出验收\Excel数据验收\交运\交通运输20250416_公路铁路_【申万交运】公路货运数据_20250512073459.xlsx'
        ...
        # 处理"2022.4.21以来交通部各项物流数据-周度"工作表
        sheet_name = "2022.4.21以来交通部各项物流数据-周度"
        source_sheet = "交通部各项物流数据"
        indicators = [
            'AA000065596100',
            'AA000065596300',
            'AA000065596500',
        ...
        # 读取所有指标数据并筛选日期
        filter_date = pd.to_datetime('2023-03-26')
        reader = DataReader(source_file)
        processor = DataProcessor(reader)
        ...
        for indicator in indicators:
            current_data = reader.read_indicator_data(source_sheet, indicator)
            sorted_data = processor.sort_by_date(current_data, ascending=True)
            df = sorted_data["data"]
            df = df[df['日期'] >= filter_date]
        ...
        for row_idx, (_, row) in enumerate(reference_df.iterrows()):
            ...
            # 检查是否是每月第一行
            is_first_of_month = (row_idx == 0 or 
                               row['日期'].month != reference_df.iloc[row_idx-1]['日期'].month)
            
            # 写入所有指标数据
            for i, df in enumerate(filtered_data):
                value = df.loc[df['日期'] == row['日期'], df.columns[-1]].iloc[0]
                ws.cell(row=current_row, column=start_col_num + i + 1, value=value)
                
                # 如果是每月第一行，设置灰色背景
                if is_first_of_month:
                    for col in range(start_col_num, start_col_num + len(indicators) + 1):
        ...
        # 设置日期列格式
        for row in range(start_row, start_row + len(reference_df)):
            ws.cell(row=row, column=start_col_num).number_format = 'yyyy-mm-dd'
