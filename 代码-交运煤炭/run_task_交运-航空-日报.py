...
def create_backup_file(original_file: str) -> str:
    """为目标文件创建带时间戳的副本，返回副本路径，并统一放入快递目标文档文件夹"""
    file_dir = os.path.dirname(original_file)
    file_name = os.path.basename(original_file)
    name, ext = os.path.splitext(file_name)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    target_folder = os.path.join(file_dir, "快递目标文档")
    if not os.path.exists(target_folder):
        os.makedirs(target_folder)
    backup_file = os.path.join(target_folder, f"{name}_{timestamp}{ext}")
    ...
    try:
        ...
        with timing_stats.timing("打开目标文件副本"):
            ...
            if target_sheet not in wb_target.sheetnames:
                wb_target.create_sheet(target_sheet)
...
def replace_zeros_with_none(backup_file, sheet_name, start_row, start_col, nrows, ncols):
    import openpyxl
    wb = openpyxl.load_workbook(backup_file)
    ws = wb[sheet_name]
    if isinstance(start_col, str):
    ...
    for i in range(nrows):
        for j in range(ncols):
            cell = ws.cell(row=start_row + i, column=start_col_num + j)
            if cell.value == 0:
                cell.value = None
    wb.save(backup_file)
...
if __name__ == "__main__":
    try:
        # 文件路径
        source_file = "D:\申万研究所实习\交运、煤炭\交运\公路\交通运输20250416_快递_【申万交运】快递数据库_20250512071459.xlsx"
        target_file = "D:\申万研究所实习\交运、煤炭\交运\公路\交运快递目标文档.xlsx"
        # 创建一个副本
        backup_file = create_backup_file(target_file)
        ...
        # 读取第一个指标的数据并将日期写入A列
        reader = DataReader(source_file)
        df = reader.read_indicator_data("四家月度数据", "AA000058887000")['data']
        wb_target = openpyxl.load_workbook(backup_file)
        ws_target = wb_target["四家月度数据"]
        for row_idx, date in enumerate(df['日期']):
            ws_target.cell(row=14 + row_idx, column=1, value=date)
        wb_target.save(backup_file)
        ...
        # 处理“四家月度数据”工作表（源表：公司＿四家月度数据，目标表：四家月度数据）
        process_type1_task(
            source_file=source_file,
            target_file=target_file,
            source_sheet="公司＿四家月度数据",
            target_sheet="四家月度数据",
            indicators=[
                "AA000058887000",
                "AA000058887200",
